...
object WordCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result1 = planBuilder1
      ...
      .map(_ * 1)
      .runAsync(tempFileOut = "file:///app/output/async.wordcount.out1.temp")

    val result2 = planBuilder2
      .loadCollection(List(6, 7, 8, 9, 10))
      .filter(_ <= 8)
      .runAsync("file:///app/output/async.wordcount.out2.temp")
    ...
    val result3 = dq1.union(dq2)
      .map(_ * 3)
      .filter(_ < 100)
      .runAsync("file:///app/output/async.wordcount.out3.temp", result1, result2)

    val result4 = planBuilder3
      .loadCollection(List(1, 2, 3, 4, 5))
      .filter(_ >= 2)
      .runAsync("file:///app/output/async.wordcount.out4.temp")
    ...
    val result5 = dq3.intersect(dq4)
      .map(_ * 4)
      .writeTextFileAsync("file:///app/output/async.wordcount.out5.final", result3, result4)
